...
use termion::color;
use crate::{highlighting, filetype::HighlightingOptions};
...
impl Row {
    ...
    pub fn render(&self, start: usize, end: usize) -> String {
            ...
            .take(end - start)
        {
            if let Some(c) = grapheme.chars().next() {
                let highlighting_type: &highlighting::Type = 
                    self.highlighting.get(index).unwrap_or(&highlighting::Type::None);
        ...
        let end_highlight: String = format!("{}", termion::color::Fg(color::Reset));
    ...
    fn highlight_char(
        ...
        if opts.characters() && c == '\'' {
            if let Some(next_char) = chars.get(index.saturating_add(1)) {
                let closing_index: usize = if *next_char == '\\' {
    ...
    pub fn highlight(
        ...
        let mut in_ml_comment: bool = start_with_comment;
        if in_ml_comment {
